...
In React, "controlled input" refers to a form input element whose value is controlled by React through state, while "uncontrolled input" refers to an input element whose value is managed by the browser DOM.

A controlled input provides a way to manage the value of the input field in the component state, allowing for easier manipulation and validation of the input value. In contrast, an uncontrolled input is managed by the browser and is typically accessed through the DOM.

In general, it's recommended to use controlled inputs in React for better control and predictability over form inputs. However, there are some situations where uncontrolled inputs may be appropriate, such as when you want to allow the user to enter any value without validation, or if you need to access the input value directly through the DOM.
...
[FormData API Readme](FormDataApi.md)
